```c++
//#3 — Store Row Sums in an Array
#include <iostream>
#include <cmath>

using namespace std;

int RandomNumber(int From, int To)
{
    int RanNum = rand() % (To - From + 1) + From;
    return RanNum;
}
void FillMatrixWithRandomNumber(int arr[3][3], int Rows, int Clos)
{
    for (int i = 0; i < Rows; i++)
    {
        for (int j = 0; j < Clos; j++)
        {
            arr[i][j] = RandomNumber(1, 100);
        }
    }
}
void PrintMatrix(int arr[3][3], int Rows, int Clos)
{
    for (int i = 0; i < Rows; i++)
    {
        for (int j = 0; j < Clos; j++)
        {
            cout << arr[i][j] << "\t";
        }
        cout << endl;
    }
}
int RowSum(int arr[3][3], int RowNumber, int Cols)
{
    int sum = 0;

    for (int j = 0; j < Cols; j++)
    {
        sum = sum + arr[RowNumber][j];
    }

    return sum;
}
void SumMatrixRowsInArray(int arr[3][3], int arrSum[3], int Rows, int Cols)
{
    for (int i = 0; i < Rows; i++)
    {
        arrSum[i] = RowSum(arr, i, Cols);
    }
}
void PrintEachRowSumArray(int arrSum[3], int Rows, int Cols)
{
    for (int i = 0; i < Rows; i++)
    {
        cout << "Row " << i + 1 << " sum : " <<arrSum[i] << endl;
    }
}
int main()
{
    srand((unsigned)time(NULL));

    int arr[3][3];
    int arrSum[3];

    FillMatrixWithRandomNumber(arr, 3, 3);

    cout << "The following is a 3*3 random matrix : " << endl;
    PrintMatrix(arr, 3, 3);
    
    SumMatrixRowsInArray(arr, arrSum, 3, 3);

    cout << "The following are the sum of each rows in the matrix : " << endl;
    PrintEachRowSumArray(arrSum, 3, 3);

    return 0;
}

```
